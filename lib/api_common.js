...
API.prototype.getAccessToken = function (callback) {
  ...
  console.log('开始从后台获取accessToken');
  var url = 'http://wechat.lrh-test.vcredit.com/api/lrh/weChatInteractive/getWechatAccessToken';
  this.request(url, {dataType: 'json'}, wrapper(function (err, data) {
    ...
    console.log('获取accessToken成功'+data.data.accessToken);
    var expireTime = (new Date().getTime()) -1000;
    var token = AccessToken(data.data.accessToken, expireTime);
